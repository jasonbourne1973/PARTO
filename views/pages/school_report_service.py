...
class SchoolReportService(BaseService):
    ...
    def generate_school_report_data(self, profile_id):
        ...
        try:
            ...
            # دریافت تفسیرهای تخصصی
            interpretations = self.interpretation_dal.get_by_student_profile(profile_id)
    ...
    def export_school_report_pdf(self, profile_id, file_path):
        ...
        try:
            ...
            # ===== تفسیر تخصصی =====
            if report_data['has_interpretations']:
                pdf.add_subtitle("تفسیر تخصصی")
                for interp in report_data['interpretations']:
                    if interp.is_final:
                        pdf.add_bold(f"عنوان: {interp.title}")
                        pdf.add_text(f"سطح: {interp.level_display}")
                        pdf.add_text(f"تفسیر: {interp.detailed_text}")
                        if interp.recommendations:
                            pdf.add_text(f"توصیه‌ها: {interp.recommendations}")
                        if interp.next_steps:
                            pdf.add_text(f"اقدامات بعدی: {interp.next_steps}")
                        pdf.add_spacer(0.2)
            else:
                pdf.add_text("تفسیر تخصصی ثبت نشده است.")
            pdf.add_spacer(0.3)
