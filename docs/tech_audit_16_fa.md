...
# بخش ۱۱ — داوری سوم مدیر پروژه (روی `24bc738`): برنامه و مرحلهٔ ۱

داور چهار ایراد تازه ثبت کرده است. برنامهٔ چندمرحله‌ای (هر مرحله با تأیید کاربر):
...
| ۴ | «تست واقعی» | اجرای کامل باتری، به‌روزرسانی گزارش/README/PR + چک‌لیست GUI برای داور | **انجام شد** (بخش ۱۴) |

## [BUG-NEW-01]
### بخش
```text
views/widgets/recommendation_widget.py — reject_recommendation / complete_recommendation
```
### وضعیت
`BROKEN` (P1 — دو دکمه در زمان کلیک با AttributeError می‌شکستند)
### اصلاح
`from PySide6.QtWidgets import QInputDialog`؛ هر دو مسیر با
`QInputDialog.getText(parent, title, label)`. (در Inventory ایستای مرحلهٔ ۳ این
دو handler «BACKEND» طبقه‌بندی شده بودند و در اجرای headless چون نیاز به شیء
ردیف داشتند و ویجت آن زمان سوار نبود، اجرا نشده بودند — درس: هر handler
دکمه‌های داخل آیتم‌های تولیدشده در حلقه هم باید با شیء واقعی اجرا شود.)
### تست
verify16 J1: انصراف → بدون تغییر؛ رد با دلیل → `status=rejected` و
`feedback_notes`؛ پذیرش → اجرا → تکمیل با بازخورد → `status=completed` و
`feedback`؛ بدون پیام خطا؛ منبع بدون `QMessageBox.getText`.
```text
STATUS: FIXED
```
...
### بخش
```text
views/widgets/recommendation_widget.py — action_label ؛ views/pages/dashboard_page.py — ۴ نشان (badge)
```
### وضعیت
`BROKEN` (P2 — متن نامرئی)
### مشکل
اسکن کنتراست مرحلهٔ ۳ فقط بلوک‌های `{...}` و رشته‌های تک‌خطی را می‌دید؛
استایل‌های **چندخطی بدون selector** (اعلان‌های CSS مستقیم داخل رشته) از قلم
افتاده بودند: «اقدام پیشنهادی» (`#0B2E4F` روی `#0B2E4F`) و چهار نشان داشبورد
(`#66BB6A` روی `#66BB6A` و `#0B2E4F` روی `#0B2E4F`).
### اصلاح
رنگ متن این ۵ مورد (طلایی روی سرمه‌ای، تیره روی سبز)؛ اسکن کنتراست
`verify_fixes16` §C8 اکنون این نوع استایل‌ها را هم پوشش می‌دهد و صفر مورد
زیر ۳:۱ گزارش می‌کند.
### تست
verify16 J2 و C8.
```text
STATUS: FIXED
...
## آزمون پس از مرحلهٔ ۱ داوری سوم
...
# بخش ۱۴ — داوری سوم، مرحلهٔ ۴: جمع‌بندی

## وضعیت چهار ایراد داوری سوم

| BUG | اولویت داور | STATUS | اثبات |
|---|---|---|---|
| BUG-NEW-01 `QMessageBox.getText` در رد/تکمیل پیشنهاد | P1/P2 | **FIXED** | verify16 J1 (رد با دلیل → rejected؛ پذیرش→اجرا→تکمیل → completed) |
| BUG-NEW-02 Migrationها خودشان commit می‌کردند | P1 | **FIXED** | verify16 K1–K4 (۰ commit در ۹ فایل؛ فایل واقعی با شکست وسط کار → هیچ‌چیز نمی‌ماند؛ زنجیرهٔ ۵→۹ و ۹→۸→۹؛ مسیر ترمیم) |
| BUG-NEW-03 متن «اقدام پیشنهادی» هم‌رنگ زمینه | P2 | **FIXED** (+۴ نشان نامرئی داشبورد) | verify16 J2 + C8 (اسکن کنتراست شامل استایل‌های بدون selector) |
| BUG-NEW-04 «ثبت مداخله از روی پیشنهاد» بدون انتقال داده/پیوند | P2 | **FIXED** | verify16 L1–L2 |
| نکتهٔ ۳ (downgrade) | 🟡 | **FIXED** با BUG-NEW-02 | verify16 K3 |
| «تست واقعی GUI» | ⚠️ | **بر عهدهٔ داور** | `docs/gui_test_checklist_fa.md` (۴۰ مسیر با پس‌شرط قابل مشاهده) |

## آنچه داور خودش می‌تواند بلافاصله اجرا کند
```bash
python -m venv venv && source venv/bin/activate
pip install -r requirements.txt
python -m pytest -q tests                # 26 passed
python verify_fixes16.py                 # ۹۳ موفق / ۰ ناموفق (دیتابیس موقت؛ بدون دست‌زدن به دادهٔ واقعی)
python verify_fixes.py … verify_fixes15.py   # جمعاً ۵۵۳ بررسی دیگر
ruff check .                              # All checks passed
```
روی سیستم بدون سرور نمایش: `QT_QPA_PLATFORM=offscreen` (روی لینوکس بدون
کتابخانه‌های Qt ممکن است به `libGL/libxkbcommon/libdbus` نیاز باشد).

## نتیجهٔ نهایی باتری در این محیط (پس از داوری سوم)

| مجموعه | نتیجه |
|---|---|
| `pytest -q tests` | 26 passed |
| verify_fixes 1 … 15 | 17 / 32 / 50 / 36 / 25 / 34 / 30 / 62 / 38 / 37 / 30 / 40 / 32 / 46 / 44 — همه سبز |
| **verify_fixes16** | **93 / 93** |
| `ruff check .` | All checks passed |
| جمع | **۶۴۶ بررسی** |

## تفکیک صادقانه (به درخواست داور)
- **CODE PATH اصلاح‌شده و در همین محیط اجرا شده (offscreen، با پیش/پس‌شرط DB/فایل):**
  همهٔ ۴۵ ایراد (۴۱ + ۴ داوری سوم).
- **REAL GUI EXECUTION تأییدنشده:** کلیک واقعی موس، دیالوگ‌های modal/تأیید،
  file picker، بازکردن فایل با برنامهٔ خارجی، اجرای دوبارهٔ برنامه پس از خروج،
  پنجرهٔ خطای راه‌اندازی — چک‌لیست بخش‌بندی‌شده در `docs/gui_test_checklist_fa.md`.
- **اجرای واقعی روی Python 3.9/3.13:** انجام نشده (دانلود مفسر در این محیط
  ممکن نبود)؛ ادعا بر پایهٔ `vermin` + حذف `makeSuite`.

## چیزهایی که هنوز عمداً دست‌نخورده‌اند (تصمیم/طراحی)
...
## مرحله ۳ — پایداری اطلاعات خانوادگی دانش‌آموز (BUG-GUI-02)

### مشکل (تأییدشده روی همین نسخه)
فرم ثبت/ویرایش دانش‌آموز سه ورودی خانوادگی داشت
(«تعداد برادران»، «تعداد خواهران»، «وضعیت زندگی») و آن‌ها را روی
`models/student.py` می‌گذاشت:
...
اما جدول `students` هیچ‌یک از این ستون‌ها را ندارد و `StudentDAL` هم
آن‌ها را نمی‌نویسد؛ نتیجه: پیام «دانش‌آموز با موفقیت ثبت/ویرایش شد»
دیده می‌شد ولی داده هرگز ذخیره نمی‌شد و در ویرایش بعدی هم
(چون از همان فیلدهای حافظه‌ای خوانده می‌شد) صفر/خالی نمایش داده می‌شد.

### تصمیم طراحی — جای قانونی داده
**`family_contexts` (نگاشت به پروندهٔ سالانه) جای قانونی این داده است؛
هیچ ستون/جدول تازه‌ای ساخته نشد.** دلیل:

- `family_contexts.siblings_brothers` و `siblings_sisters` از قبل وجود دارند.
- «وضعیت زندگی» همان مفهوم `guardian_status` است؛ نگاشت دوطرفه بین متن
  فرم (`config.constants.LIVING_STATUSES`) و کدهای
  `FamilyContext.GUARDIAN_CHOICES` اضافه شد
  (`FamilyContext.LIVING_STATUS_MAP` / `GUARDIAN_TO_LIVING_STATUS` و
  پروپرتی `living_status`).
- اطلاعات خانوادگی **به پروندهٔ همان سال** می‌چسبد؛ پس سال‌های مختلف
  یک دانش‌آموز دادهٔ مستقل دارند و هیچ نشتی بین سال‌ها رخ نمی‌دهد.
- مدل `Student` دیگر هیچ نسخهٔ سایه‌ای از این سه فیلد ندارد
  (منبع باگ).

### اصلاح‌ها
| فایل | تغییر |
|---|---|
| `models/student.py` | حذف فیلدهای سایه‌ای + یادداشت جای قانونی |
| `models/family_context.py` | نگاشت «وضعیت زندگی» ↔ `guardian_status`؛ مقدار ناشناخته/خالی هیچ‌وقت داده را پاک نمی‌کند |
| `dal/family_context_dal.py` | متد `upsert_family_facts(profile_id, living_status, brothers, sisters)`: تک‌ردیفی، بررسی واقعی `rowcount`، بازخوانی از دیتابیس، و رد کردن حالت چندردیفی/عدم مطابقت با `RuntimeError` |
| `views/dialogs/student_form.py` | خواندن از دیتابیس در ویرایش (`FamilyContextDAL.get_by_student_profile`) و ذخیره در همان جای قانونی با `upsert_family_facts` |
| `views/pages/student_profile_page.py` | نمایش «وضعیت زندگی» و «خواهر/برادر» در کارت زمینهٔ خانوادگی (مسیر خوانش دیده‌شدنی) |

### تست و شاهد
- تست‌های واحد: `tests/test_dal.py::TestFamilyContextDAL` (۶ تست) —
  ثبت/بازخوانی، به‌روزرسانی همان ردیف (بدون تکرار)، حفظ مقدار ناشناخته،
  نبود فیلد سایه روی `Student`، استقلال هر پرونده، رد `profile_id` خالی.
- `verify_fixes16.py` بخش **M** (۴ بررسی) با مسیر واقعی فرم:
  ذخیره → بازخوانی در فرم → ویرایش → یک ردیف؛ و پایداری پس از بستن
  کامل اتصال‌ها (شبیه‌سازی راه‌اندازی دوباره).
- **اثبات معکوس:** با بازگرداندن پنج فایل مرحلهٔ ۳ به نسخهٔ قبل، سه بررسی
  M1..M3 شکست می‌خورند؛ گزارش دقیقاً نشان می‌دهد
  `row=None rows=0 read_back=(0, 0, 'با هر دو والدین')` در حالی که پیام
  «دانش‌آموز با موفقیت ویرایش شد» نمایش داده شده است (همان موفقیت دروغین).
- شاهد دیده‌شدنی: کارت زمینهٔ خانوادگی پروندهٔ دانش‌آموز:
  `وضعیت زندگی: فقط با پدر | خواهر/برادر: 1 خواهر، 2 برادر | …`

## مرحله ۴ — همگام‌سازی «سال تحصیلی فعال» (BUG-GUI-12 / NAV-04/05/06)

### مشکل (تأییدشده روی همین نسخه)
- `MainWindow.academic_year_changed` سیگنالی **بدون هیچ گیرنده** بود.
- صفحه‌ها با **بازتاب** دوباره بارگذاری می‌شدند: `dir(page)` روی همهٔ متدهای
  بی‌آرگومانِ `load_*` (شامل `inspect.signature`). یعنی هیچ صفحه‌ای «سال
  انتخاب‌شده» را نمی‌گرفت؛ همه جداگانه `academic_year_dal.get_active()` را
  می‌خواندند و اگر سال فعال جای دیگری عوض می‌شد، هر صفحه چیز دیگری نشان
  می‌داد (نشتی/ناسازگاری بین صفحه‌ها).
- کامبوی سال صفحه‌ها با سال انتخابی همگام نمی‌شد و کامبوی هدر هنگام
  پرکردن، سیگنال `currentIndexChanged` می‌فرستاد؛ یعنی ممکن بود همان
  لحظهٔ باز شدن برنامه دیالوگ «سال فعال تغییر کند؟» ظاهر شود.

### اصلاح‌ها
| فایل | تغییر |
|---|---|
| `views/pages/year_sync.py` (جدید) | `select_year_in_combo` (انتخاب بدون سیگنال)، کلاس `YearAwarePage` با `set_active_year` / `reload_for_year` / `effective_year` |
| `views/main_window.py` | `set_active_year(year_id, persist=True, announce=True)` = **تک‌منبع حقیقت**؛ سیگنال به `_sync_pages_for_year` وصل شد (گیرندهٔ واقعی)؛ فهرست صریح `_year_aware_pages()`؛ **حذف کامل اسکن بازتابی**؛ `_reload_pages_for_year` صرفاً پوشش منسوخ؛ پرکردن کامبوی هدر بدون سیگنال |
| ۱۹ صفحهٔ `views/pages/*` | ارث‌بری از `YearAwarePage` + `reload_for_year` مخصوص خودِ همان صفحه |
| صفحه‌های وابسته به سالِ فعال (`observations`, `interventions`, `followups`, `activities`, `counseling`, `goals`) | `self.academic_year_dal.get_active()` → `self.effective_year()` (انتخاب صریح بر حدس مقدم است) |
| صفحه‌های ترکیبی | `students_page` → پروندهٔ دانش‌آموز؛ `dashboard_page` → تحلیل‌ها؛ `reports_page` → گزارش کلاس/معلم/عملکرد؛ `academic_structure_page` → تب ارتقاء پایه |

### چرا `effective_year` «حدس» نیست
ترتیب قطعی است: **انتخاب صریح سامانه** → و فقط در نبود آن، سال فعال
دیتابیس. در عمل `MainWindow.set_active_year` هر دو را هم‌راستا می‌کند، ولی
صفحه دیگر هرگز خودش سراغ «آخرین سال فعال دیتابیس» نمی‌رود.

### تست و شاهد
- `tests/test_year_sync.py` (۱۴ تست): انتخاب بدون سیگنال، تازه‌سازی فهرست
  سال‌ها برای سال تازه‌ساخته‌شده، تقدم انتخاب صریح، سیم‌کشی کد (نبود
  `inspect`، ارث‌بری همهٔ صفحه‌ها، فهرست صریح، نبود `get_active` در
  صفحه‌های وابسته) و تعامل با دیتابیس واقعی.
- `verify_fixes16.py` بخش **N** (۴ بررسی): تغییر سال روی پنجرهٔ واقعی
  → سال فعال دیتابیس عوض می‌شود، سیگنال گیرندهٔ واقعی دارد، **هر ۱۳ صفحه**
  همگام می‌شوند (`skipped=[]`)، کامبوی هر صفحه روی سال جدید است،
  زیرصفحه‌های جاسازی‌شده هم همگام‌اند، هیچ پیام خطایی ساخته نمی‌شود،
  سال فعال بایگانی‌شده رد می‌شود، و اگر سال فعال دیتابیس بیرون از برنامه
  عوض شود صفحه‌ها همچنان سال اعلام‌شده را می‌خوانند.
